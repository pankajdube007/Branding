...
namespace GoldMedal.Branding.Admin.Master.Unit
{
    public partial class Unit : System.Web.UI.Page
    {
        ...
        private string TableNm = "Tbl_Unit";
        ...
        protected void DataInsert()
        {
            ...
            Data.Unit.UnitModel.UnitInsert dst = new Data.Unit.UnitModel.UnitInsert();
            ...
            else
            {
                ...
                if (lbslno.Text == "0")
                {
                    Core.Unit.Unit objinsert = new Core.Unit.Unit();
                    result = objinsert.UnitInsertMethod(dst);
                    if (result == 2)
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Unit already exists !','warning',3);", true);
                        clean();
                    }
                    else if (result == 1)
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Unit added successfully !','success',3);", true);
                        clean();
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Unit Updated successfully !','success',3);", true);
                        ResetFunc();
                        clean();
                    }
                    else
        ...
        {
            return string.Format("Master_Unit_{0}", DateTime.Now.ToString());
        }
        ...
        /// <summary>
        /// returns the datatable which contains the all Unit records
        /// </summary>
        /// <returns></returns>
        private DataTable GetTable1()
        {
            DataTable dt4 = new DataTable();
            Core.Unit.Unit objselectall = new Core.Unit.Unit();
            dt4 = objselectall.GetUnitAll();
            return dt4;
        }
            ...
            if (dtEditChk.Rows.Count > 0)
            {
                if (dtEditChk.Rows[0]["PStatus"] != null && (dtEditChk.Rows[0]["PStatus"].ToString() == "InActive" || dtEditChk.Rows[0]["PStatus"].ToString() == "Unchecked"))
                {
                    if (param == "Delete")
                    {
                        ...
                        Data.Unit.UnitModel.UnitDelete ddel = new Data.Unit.UnitModel.UnitDelete();
                        ...
                        Core.Unit.Unit objdelete = new Core.Unit.Unit();
                        rows = objdelete.UnitDeleteMethod(ddel);
                        if (rows == 1)
                        {
                    ...
                    else
                    {
                        ...
                        Data.Unit.UnitModel.UnitInsert dtsingle = new Data.Unit.UnitModel.UnitInsert();
                        dtsingle.slno = Convert.ToInt32(lbslno.Text);
                        Core.Unit.Unit objselectsingle = new Core.Unit.Unit();
                        DataTable dt = objselectsingle.GetUnitSingle(dtsingle);
                        if (dt.Rows.Count > 0)
                        {
