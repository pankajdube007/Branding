...
{
    public partial class Fabricator : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// returns the datatable which contains the all Fabricator records
        /// </summary>
        /// <returns></returns>
        private DataTable GetTable1()
        {
            ...
            dt4 = objselectall.GetFabricatorAll();
            return dt4;
        }
        ...
        protected void DataInsert()
        {
            ...
            else
            {
                ...
                {
                    dst.usernm = "fab_" + username;
                }
                else
                {
                    
                    if (!username.Contains("fab_"))
                    {
                        dst.usernm = "fab_" + username;
                    }
                    else
                ...
                result = objinsert.FabricatorInsertMethod(dst);
                if (result == -2)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Fabricator already exists !','warning',3);", true);
                    ResetFunc();
                    clean();
                }
                else if (result == -1)
                ...
                {
                    Data.Fabricator.FabricatorModel.FabricatorInsert dstt = new Data.Fabricator.FabricatorModel.FabricatorInsert();
                    foreach (GridViewRow row in gvSchemeChild.Rows)
                    {
                        ...
                        dstt.unitid = unitid;
                        result2 = objinsert.fabricatormaterialSubmitupdate(dstt);
                        if (result2 == -1)
                        {
                            Data.Fabricator.FabricatorModel.FabricatorDelete del = new Data.Fabricator.FabricatorModel.FabricatorDelete();
                            del.slno = Convert.ToInt32(result);
                            errorresulr = objinsert.PermanentDeletefab(del);
                            ResetFunc();
                            clean();
                        ...
                        {
                            ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Fabricator and material details added successfully !','success',3);", true);
                        }
                        else if (result2 == 3)
                        {
                            ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Fabricator and material details updated successfully  !','success',3);", true);
                        }
                        else
                        {
                        ...
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Fabricator and its material added successfully!','success',3);", true);
                    }
                    ResetFunc();
                    clean();
                    ASPxGridView1.DataBind();
                ...
                else
                {
                    if (lbslno.Text == "0")
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','No Materials added only Fabricator added successfully !','success',3);", true);
                    }
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','No Materials updated only Fabricator updated successfully !','success',3);", true);
                    }
                    ResetFunc();
                    clean();
                    ASPxPageControl1.ActiveTabIndex = 1;
            ...
            if (dtEditChk.Rows.Count > 0)
            {
                if (dtEditChk.Rows[0]["PStatus"] != null && (dtEditChk.Rows[0]["PStatus"].ToString() == "InActive" || dtEditChk.Rows[0]["PStatus"].ToString() == "Unchecked"))
                {
                    if (param == "Delete")
                    {
                        ...
                        rows = objdelete.FabricatorDeleteMethod(ddel);
                        if (rows == 1)
                        {
        ...
        {
            return string.Format("Master_Fabricator_{0}", DateTime.Now.ToString());
        }

        private string[] Validationchild()
        {
            string[] results = new string[2];
            if (cmbMaterial.Value == string.Empty || Convert.ToDecimal(txtrate.Text) <= 0 || Convert.ToDecimal(cmbunit.Value) <= 0)
            {
                results[0] = "true";
        ...
        private void loadchild()
        {
            ...
            Core.Fabricator.Fabricator objselectall = new Core.Fabricator.Fabricator();
            dt6 = objselectall.GetDetailOfMaterialListForFabricator(dchildlist);
        ...
        private void BindSuppliers()
        {
            ...
            DataTable suppliers = da.AllSupplierListFabricatorDA();
        ...
        {
            DataTable supplier = da.GetSupplierDetailsFab(SupplierID);
        ...
        protected void gvSchemeChild_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            Data.Fabricator.FabricatorModel.FabricatorDelete param1 = new Data.Fabricator.FabricatorModel.FabricatorDelete();
            //child srno
            param1.slno = Convert.ToInt32(((HiddenField)gvSchemeChild.Rows[index].FindControl("hfchildslno")).Value);
            Core.Fabricator.Fabricator objdelete = new Core.Fabricator.Fabricator();
            rows = objdelete.Deletefabricatormaterial(param1);
            if (rows == 1)
            {
