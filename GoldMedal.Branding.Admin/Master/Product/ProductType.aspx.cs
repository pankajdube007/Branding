...
namespace GoldMedal.Branding.Admin.Master
{
    public partial class ProductType : System.Web.UI.Page
    {
        ...
        private string TableNm = "Tbl_ProductType";
        ...
        /// <summary>
        /// returns the datatable which contains the all product type records
        /// </summary>
        /// <returns></returns>
        private DataTable GetTable1()
        {
            DataTable dt4 = new DataTable();
            Core.ProductType.ProductType objselectall = new Core.ProductType.ProductType();
            dt4 = objselectall.GetProductTypeAll();
            return dt4;
        }
        ...
        protected void DataInsert()
        {
            ...
            Data.ProductType.ProductTypeModel.ProductTypeInsert dst = new Data.ProductType.ProductTypeModel.ProductTypeInsert();
            ...
            else
            {
                ...
                if (lbslno.Text == "0")
                {
                    Core.ProductType.ProductType objinsert = new Core.ProductType.ProductType();
                    result = objinsert.ProductTypeInsertMethod(dst);
                    if (result == 2)
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Product Type already exists !','warning',3);", true);
                        clean();
                    }
                    else if (result == 1)
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Product Type added successfully !','success',3);", true);
                        clean();
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Product Type Updated successfully !','success',3);", true);
                        ResetFunc();
                        clean();
                    }
                    else
            ...
            if (dtEditChk.Rows.Count > 0)
            {
                if (dtEditChk.Rows[0]["PStatus"] != null && (dtEditChk.Rows[0]["PStatus"].ToString() == "InActive" || dtEditChk.Rows[0]["PStatus"].ToString() == "Unchecked"))
                {
                    if (param == "Delete")
                    {
                        ...
                        Data.ProductType.ProductTypeModel.ProductTypeDelete ddel = new Data.ProductType.ProductTypeModel.ProductTypeDelete();
                        ...
                        Core.ProductType.ProductType objdelete = new Core.ProductType.ProductType();
                        rows = objdelete.ProductTypeDeleteMethod(ddel);
                        if (rows == 1)
                        {
                            ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Product Type deleted successfully !','success',3);", true);
                    ...
                    else
                    {
                        ...
                        Data.ProductType.ProductTypeModel.ProductTypeInsert dtsingle = new Data.ProductType.ProductTypeModel.ProductTypeInsert();
                        dtsingle.slno = Convert.ToInt32(lbslno.Text);
                        Core.ProductType.ProductType objselectsingle = new Core.ProductType.ProductType();
                        DataTable dt = objselectsingle.GetProductTypeSingle(dtsingle);
                        if (dt.Rows.Count > 0)
                        {
        ...
        {
            return string.Format("Master_ProductType_{0}", DateTime.Now.ToString());
        }
