...
namespace GoldMedal.Branding.Admin.Master.JobType
{
    public partial class JobType : System.Web.UI.Page
    {
        ...
        private string TableNm = "Tbl_JobType";
        ...
        protected void btnOverWrite_Click(object sender, EventArgs e)
        {
            ...
            Data.JobType.JobTypeModel.JobTypeInsert dtsingle = new Data.JobType.JobTypeModel.JobTypeInsert();
            dtsingle.slno = Convert.ToInt32(lbslno.Text);
            Core.JobType.JobType objselectsingle = new Core.JobType.JobType();
            DataTable dt = objselectsingle.GetJobTypeSingle(dtsingle);
            if (dt.Rows.Count > 0)
            {
                txtName.Text = Convert.ToString(dt.Rows[0]["name"]);
                lbslno.Text = Convert.ToString(dt.Rows[0]["slno"]);
                if (Convert.ToString(dt.Rows[0]["isimgreq"]) == "false")
                {
                    chk.Checked = false;
                }
                else
        ...
        protected void DataInsert()
        {
            ...
            bool img;
            bool apr;
            Data.JobType.JobTypeModel.JobTypeInsert dst = new Data.JobType.JobTypeModel.JobTypeInsert();
            ...
            else
            {
                if (chk.Checked == true)
                {
                    img = Convert.ToBoolean(1);
                }
                else
                {
                    img = Convert.ToBoolean(0);
                }
                if (chkapp.Checked == true)
                {
                    apr = Convert.ToBoolean(1);
                }
                else
                {
                    apr = Convert.ToBoolean(0);
                }
               ...
               // dst.ImageValidToDate = ToDate;
                ...
                if (lbslno.Text == "0")
                {
                    Core.JobType.JobType objinsert = new Core.JobType.JobType();
                    result = objinsert.JobTypeInsertMethod(dst);
                    if (result == 2)
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Job Type already exists !','warning',3);", true);
                        clean();
                    }
                    else if (result == 1)
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Job Type added successfully !','success',3);", true);
                        clean();
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Job Type updated successfully !','success',3);", true);
                        ResetFunc();
                        clean();
                    }
                    else
        ...
        private DataTable GetTable1()
        {
            ...
            dt4 = objselectall.GetJobTypeAll();
            return dt4;
        }
            ...
            if (dtEditChk.Rows.Count > 0)
            {
                if (dtEditChk.Rows[0]["PStatus"] != null && (dtEditChk.Rows[0]["PStatus"].ToString() == "InActive" || dtEditChk.Rows[0]["PStatus"].ToString() == "Unchecked"))
                {
                    if (param == "Delete")
                    {
                        ...
                        Data.JobType.JobTypeModel.JobTypeDelete ddel = new Data.JobType.JobTypeModel.JobTypeDelete();
                        ...
                        Core.JobType.JobType objdelete = new Core.JobType.JobType();
                        rows = objdelete.JobTypeDeleteMethod(ddel);
                        if (rows == 1)
                        {
                    ...
                    else
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            hfReferSheet.Text = Convert.ToString(dt.Rows[0]["JobTypeImage"]);
                            ...
                            if (Convert.ToString(dt.Rows[0]["isaprreq"]) == "False")
                            {
                                chkapp.Checked = false;
                            }
                            else
        ...
        {
            return string.Format("Master_JobType_{0}", DateTime.Now.ToString());
        }
