...
{
    public partial class apdapdesignaprvdbymanag : System.Web.UI.Page
    {
        ...
        public apdapdesignaprvdbymanag()
        {
        ...
        {
            int role = 0;
            role = GoldMedal.Branding.Core.Common.ValidateDataType.GetCookieInt("userlogid");
           // if (role == 1) 
            if (role == 421)
            {
                ASPxGridView2.Columns[25].Visible = true;
            }
            else
            {
                ASPxGridView2.Columns[25].Visible = false;
            }
        }
        #region Events
        ...
        private DataTable GetApprovedJobs()
        {
            ...
            dsp.createuid = GoldMedal.Branding.Core.Common.ValidateDataType.GetCookieInt("userlogid");
            Core.DesignSubmit.DesignSubmit objselectall = new Core.DesignSubmit.DesignSubmit();
            dt4 = objselectall.GetAllDesignApprovedByMgm(dsp);
            return dt4;
        }
        ...
        protected void ASPxGridView2_HtmlRowCreated(object sender, ASPxGridViewTableRowEventArgs e)
        {
            ...
            LinkButton lbaccept = ASPxGridView2.FindRowCellTemplateControl(e.VisibleIndex, ASPxGridView2.Columns["Accept Job"] as GridViewDataTextColumn, "CmdAccept") as LinkButton;
            if (lbaccept != null)
            {
        ...
        protected void lnkShowImg_Command(object sender, CommandEventArgs e)
        {
            ...
            GetUploadedJobRequestFiles(Convert.ToInt32(lblImageSlno.Text), 4);
            this.mpeImage1.Show();
        }

        protected void ASPxGridView1_DataBinding(object sender, EventArgs e)
        {
            ASPxGridView1.DataSource = GetAcceptedJobs();
        }

        private DataTable GetAcceptedJobs()
        {
            DataTable dt4 = new DataTable();
            Data.DesignSubmit.DesignSubmit.DesignSubmitProperty dsp = new Data.DesignSubmit.DesignSubmit.DesignSubmitProperty();
            dsp.createuid = GoldMedal.Branding.Core.Common.ValidateDataType.GetCookieInt("userlogid");
            Core.DesignSubmit.DesignSubmit objselectall = new Core.DesignSubmit.DesignSubmit();
            dt4 = objselectall.GetAllDesignAcceptedByMgm(dsp);
            return dt4;
        }
        ...
        private void Accept()
        {
            ...
            else
            {
                ...
                Core.DesignSubmit.DesignSubmit cdsub = new Core.DesignSubmit.DesignSubmit();
                int result = 0;
                result = cdsub.LiveProductjobsAcceptbymanagement(designsub);
                if (result == 1)
                {
