...
{
    public partial class ReceiveJob : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    txtJobReceiveDate.Text = DateTime.Now.Date.ToString("dd/MM/yyyy");
                    txtJobReceiveDate.MaxDate = DateTime.Now;
        ...
        private DataTable GetTable2()
        {
            ...
            dt5 = objselectall.GetJobSentByPrinterToReceive(dsp);
            return dt5;
        }
        ...
        protected void show2()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblNoofPackages.Text = Convert.ToString(dt.Rows[0]["NoofPackages"]);
        ...
        protected void Receive()
        {
            ...
            {
                if (txtJobReceiveDate.Text == "")
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Please select receive date','warning',3);", true);
                }
                else if (Convert.ToInt16(txtqty.Text) > Convert.ToInt16(lbltotalqty.Text))
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Receiving quantity cannot be greater than total quantity','warning',3);", true);
                }
                else
                {
                    bool success = false;
                    DateTime rcdate = DateTime.MinValue;
                    success = DateTime.TryParse(txtJobReceiveDate.Date.ToString("yyyy-MM-dd"), out rcdate);

                    if (!success || rcdate == DateTime.MinValue)
                    {
                        return;
                    }
                    else
                    {
                        ...
                        dst.JobReceiveDate = txtJobReceiveDate.Date.ToString("yyyy-MM-dd");
                        ...
                        dst.RecievedQty= Convert.ToInt16(txtqty.Text);
                        dst.Actiontobetakentype = Convert.ToInt16(ddlActiontobetaken.SelectedValue);

                        Core.PrinterDesignSubmit.PrinterDesignSubmit objSend = new Core.PrinterDesignSubmit.PrinterDesignSubmit();
                        result = objSend.JobReceiveMethod(dst);
                        if (result == 1)
                        {
                        ...
                        {
                            ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Error','You Have Not Changed The Quantity !','error',3);", true);
                            clean();
                        }
                        else
                        {
        ...
        protected void Reject()
        {
            ...
            else
            {
                if (txtJobReceiveDate.Text == "")
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Please select reject date','warning',3);", true);
                }
                else if (Convert.ToInt16(txtqty.Text) > Convert.ToInt16(lbltotalqty.Text))
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Rejecting quantity cannot be greater than total quantity','warning',3);", true);
                }
                else
                {
                    bool success = false;
                    ...
                    else
                    {
                        ...
                        dst.slno = Convert.ToInt32(lbslno.Text);
                        dst.JobRejectDate = txtJobReceiveDate.Date.ToString("yyyy-MM-dd");
                        dst.createlogno = GoldMedal.Branding.Core.Common.ValidateDataType.GetCookieInt("logno");
                        dst.createuid = GoldMedal.Branding.Core.Common.ValidateDataType.GetCookieInt("userlogid");
                        ...
                        if (result == 1)
                        {
                            ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Job rejected successfully !','success',3);", true);
                            clean();
                        }
                        else
                        {
