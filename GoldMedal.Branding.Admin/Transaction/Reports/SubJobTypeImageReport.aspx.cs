...
{
    public partial class SubJobTypeImageReport : System.Web.UI.Page
    {
        ...
        private DataTable GetTable2()
        {
            ...
            dt4 = objselectall.GetSubJobTypeImage();
            return dt4;
        }
        ...
        {
            return string.Format("Active_SubJob_Type_Image_List_{0}", DateTime.Now.ToString());
        }

        protected string GetFileName1()
        {
            return string.Format("Inactive_SubJob_Type_Image_List_{0}", DateTime.Now.ToString());
        }
        ...
        protected void rptAllImages_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Down")
            {
                ...
                else if (hfPopupImageFlag.Value == "20")
                {
                    path = string.Format("{0}/{1}", FILE_DIRECTORY_NAME, hfPImgName.Value);
                    Download(path);
                    GetUploadedJobRequestFiles(Convert.ToInt32(lbslno.Text), 20);
                }
        ...
        private DataTable GetTable3()
        {
            DataTable dt4 = new DataTable();
            Core.SubJobType.SubJobType objselectall = new Core.SubJobType.SubJobType();
            dt4 = objselectall.GetSubJobTypeInactiveImage();
            return dt4;
        }
        ...
        protected void lnkSubJobTypePhoto_Command(object sender, CommandEventArgs e)
        {
            int FieldTripID = Convert.ToInt32(e.CommandArgument);
            lbslno.Text = FieldTripID.ToString();

            GetUploadedJobRequestFiles(Convert.ToInt64(lbslno.Text), 18);
            this.mpeAll.Show();
        }

        protected void lnkSubJobTypePhotoInactive_Command(object sender, CommandEventArgs e)
        {
            int FieldTripID = Convert.ToInt32(e.CommandArgument);
            lbslno.Text = FieldTripID.ToString();

            GetUploadedJobRequestFiles(Convert.ToInt64(lbslno.Text), 20);
            this.mpeAll.Show();
        }
    }
}