...
{
    public partial class BillUpload : System.Web.UI.Page
    {
        ...
        protected void CmdDelete_Command(object sender, CommandEventArgs e)
        {
            int FieldTripID = Convert.ToInt32(e.CommandArgument);
            lbslno.Text = FieldTripID.ToString();
            BillUploadModel.BillUploadDelete ddel = new BillUploadModel.BillUploadDelete();
            ...
            Core.BillUpload.BillUpload objdelete = new Core.BillUpload.BillUpload();
            rows = objdelete.BillUploadDeleteMethod(ddel);
            if (rows == 1)
            {
                ASPxGridView1.DataBind();
                ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Your Record Delete Successfull !','success',3);", true);
                clean();
            }
            else
            {
        ...
        private DataTable GetTable()
        {
            ...
            dt4 = objselectsingle.GetBillUploadAll();
            return dt4;
        }
        ...
        private void Submit()
        {
            ...
            var BranchId = HttpUtility.HtmlEncode(cmbBranch.Value);
            var Type = HttpUtility.HtmlEncode(cboType.Value);
            var TypeName = HttpUtility.HtmlEncode(cboTypeName.Value);
            string DCNumber = "";
            for (int i = 0; i < cboDCNumber.Items.Count; i++)
            {
                if (cboDCNumber.Items[i].Selected == true)
                {
                    DCNumber += cboDCNumber.Items[i].Value.ToString() + ",";
                }
            }
            DCNumber = DCNumber.TrimEnd(',');

            if (FileName.Trim(',') != "" && BranchId != "Select" && DCNumber != "")
            {
                int result = 0;
                ...
                dst.BranchID = Convert.ToInt32(BranchId);
                dst.Type = Convert.ToInt32(Type);
                dst.TypeNameID = Convert.ToInt32(TypeName);
                dst.DCNumber = DCNumber;
                dst.FileNm = FileName;
                ...
                result = objinsert.BillUploadInsertMethod(dst);
                if (result == 0)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Your Record Added successfully !','success',3);", true);
                    if (lbslno.Text != "0")
                    {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Success','Your Record Updated successfully !','success',3);", true);
                    if (lbslno.Text != "0")
                    {
                        Delete(hfReferSheetDelete.Text);
                    }
                ...
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Error','Oops ! Your Record Already Added !','error',3);", true);
                    ASPxPageControl1.ActiveTabIndex = 0;
                }
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "showDialog('Warning','Please Select Mandatory Field','warning',3);", true);
            }
